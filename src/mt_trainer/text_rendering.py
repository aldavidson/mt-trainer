...
class Cv2TextRenderer:
    ...
    def render(self, string, image, top=0, left=0, font_face=cv2.FONT_HERSHEY_COMPLEX, pixel_height=10, color=(0, 0, 0), thickness=1):
        '''
          Render the given string on the given image, at the top/left position,
          in the given font_face and color, at the given pixel_height
        ...
        cv2.putText(image,
                    ...
                    cv2.getFontScaleFromHeight(
                        font_face, pixel_height, thickness),
                    color,
                    thickness,
                    cv2.LINE_AA)
    ...
    def pixel_width(self, 
                    ...
                    thickness=1):
        '''
          Return the width in pixels of the given string, when rendered in the
          given font  on the given image, at the given pixel height, with the
          given features
        '''
        font_scale = cv2.getFontScaleFromHeight(
            font_face, pixel_height, thickness)
        (value_width, _value_height), _baseline = cv2.getTextSize(
            string, font_face, font_scale, 1)
        return value_width
...
class PILTextRenderer:
    ...
    def lazy_load_font(self, font_face, font_path=None):
        ...
        if this_font is None:
            self.fonts[font_face] = ImageFont.truetype(
                (font_path or self.font_path) + font_face)
    ...
    def convert_and_render(self,
        ...
        draw.text((top, left), string, font=(
            font or self.lazy_load_font(font_face)), fill=color)
